"""The exposure contract, checked (ART-DIRECTION 2.9).  ASSAYER SPIKE COPY.

Only the in-situ frames are held to the contract. The `clear_` frames are the
concept board's studio rig, are deliberately not the game, and are reported
without a verdict so that a bright studio elevation cannot quietly become the
reason the contract is relaxed.
...
Run:  python spikes/godot/assayer/lumcheck.py
"""
import sys, os, glob
import numpy as np
...
def main():
    ...
    for f in files:
        ...
        if "clear" in name:
            print("%-30s %6.2f%% %6.2f%% %6.2f%% %6.2f%% %8.5f   (CLEAR rig -- not the game)" % (
                name[:-4], b["blown"], b["mid"], b["legible"], b["black"], b["mean"]))
            continue
        if b["blown"] > 3.0:
            v.append("BLOWN")
        if b["legible"] < 3.0:
