"""Which lines of a rendered tree a stop lit up. Functions only.
...
That is the one rule this side duplicates, and `TreePanel` cross-checks it: the
action at the end of the path it computes must equal what `induct decide` answers
for the same stop, and it says so loudly when it does not.
"""
