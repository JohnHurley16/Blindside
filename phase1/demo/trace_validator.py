"""A tiny validator for trace.json against blocks.json. The corridor's, with the one
optional field the cave adds admitted.

    python -m phase1.demo.trace_validator --blocks phase1/blocks.json TRACE.json [...]

Checks shape and the contract's rules: enabled ids come from the block list, every
step carries exactly the enabled predicates and a raw number for each enabled
parametric one, every action is enabled, params cover the enabled parametric
predicates. A step may carry `reason` (a string); nothing else extra. Names no block.

`params` is the one map that may say more than the run used. The contract's rule that
a disabled predicate is "absent from both maps" is written about a step's `predicates`
and `raw`; the contract's own trace.json example then carries a `params` entry for a
predicate that is not enabled. The writer here filters `params` to the enabled
predicates -- a demonstration should record the numbers it actually ran with -- but a
trace that carries a spare entry is accepted, so the two sides of the seam cannot
disagree about it. Anything named in `params` must still be a predicate on the block
list.
"""
...
STEP_FIELDS: tuple[str, ...] = ("tick", "junction", "predicates", "raw", "action")
OPTIONAL_STEP_FIELDS: tuple[str, ...] = ("reason",)
...
def validate(trace: dict[str, Any], blocks: dict[str, Any]) -> list[str]:
    ...
    for i, step in enumerate(trace["steps"]):
        ...
        for key in step:
            if key not in STEP_FIELDS and key not in OPTIONAL_STEP_FIELDS:
                problems.append(f"{where}: unexpected field {key!r}")
        if any(k not in step for k in STEP_FIELDS):
            continue
        ...
        if "reason" in step and not isinstance(step["reason"], str):
            problems.append(f"{where}: reason is not a string")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="phase1.demo.trace_validator")
