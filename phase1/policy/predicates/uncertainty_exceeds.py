...
def evaluate(belief: Belief, params: Mapping[str, float]) -> tuple[bool, float | None]:
    """Position sigma against the one parameter, under whatever name the block list
    gives it. The corridor's block, same estimator. The raw sigma is returned
    alongside so the threshold can be re-fitted."""
    if len(params) != 1:
